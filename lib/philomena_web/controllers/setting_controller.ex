defmodule PhilomenaWeb.SettingController do
  ...
  defp update_local_settings(conn, user_params) do
    conn
    |> set_cookie_from_params(user_params, "hidpi")
    |> set_cookie_from_params(user_params, "webm")
    |> set_cookie_from_params(user_params, "serve_webm")
    |> set_cookie_from_params(user_params, "unmute_videos")
    |> set_cookie_from_params(user_params, "chan_nsfw")
    |> set_cookie_from_params(user_params, "hide_staff_tools")
    |> set_cookie_from_params(user_params, "hide_uploader")
    |> set_cookie_from_params(user_params, "hide_score")
    |> set_cookie_from_params(user_params, "unfilter_tag_suggestions")
    |> set_cookie_from_params(user_params, "enable_search_ac")
    |> set_cookie_from_params(
      user_params,
  ...
  defp set_cookie_from_params(conn, params, name) do
    # JS wants access; max-age is set to 25 years from now
    Conn.put_resp_cookie(conn, name, params[name],
      max_age: 788_923_800,
      http_only: false,
      extra: "SameSite=Lax"
    )
