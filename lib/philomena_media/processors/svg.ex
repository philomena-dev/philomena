defmodule PhilomenaMedia.Processors.Svg do
  ...
  defp preview(file) do
    ...
    {_output, 0} = Broker.cmd("safe-rsvg-convert", [file, preview])
  ...
  defp scale(preview, {thumb_name, {width, height}}) do
    ...
    {_output, 0} =
      Broker.cmd("ffmpeg", ["-loglevel", "0", "-y", "-i", preview, "-vf", scale_filter, scaled])

    {_output, 0} = Broker.cmd("optipng", ["-i0", "-o1", "-quiet", "-clobber", scaled])
