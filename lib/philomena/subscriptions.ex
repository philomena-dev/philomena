defmodule Philomena.Subscriptions do
  ...
  defmacro __using__(opts) do
    ...
    quote do
      ...
      ## Examples
          ...
          iex> maybe_subscribe_on(multi, object, user, :watch_on_reply)
          %Ecto.Multi{}

          iex> maybe_subscribe_on(multi, object, nil, :watch_on_reply)
          %Ecto.Multi{}

      """
      def maybe_subscribe_on(multi, object, user, field) do
        Philomena.Subscriptions.maybe_subscribe_on(multi, __MODULE__, object, user, field)
      end
    end
  end
  ...
  @doc false
  def maybe_subscribe_on(multi, module, object, user, field)
      when field in [:watch_on_reply, :watch_on_upload, :watch_on_new_topic] do
    case user do
      %{^field => true} ->
        Multi.run(multi, :subscribe, fn _repo, _changes ->
